...
Date.parseString = function(val, format, lenient) {
	...
	if (typeof(format) == "undefined" || format == null || format == "") {
		var generalFormats = new Array(Date.defaultFormat, 'y-M-d', 'MMM d, y',
			'MMM d,y', 'y-MMM-d', 'd-MMM-y', 'MMM d', 'MMM-d', 'd-MMM');
		var monthFirst = new Array('M/d/y', 'M-d-y', 'M.d.y', 'M/d', 'M-d');
		var dateFirst = new Array('d/M/y', 'd-M-y', 'd.M.y', 'd/M', 'd-M');
		var checkList = new Array(generalFormats, Date.preferAmericanFormat ?
			monthFirst : dateFirst, Date.preferAmericanFormat ? dateFirst : monthFirst
		);
	...
	this.decodeShortcut = function(str) {
		...
		} else if (["W", "TW", "WEEK", "THISWEEK", "WEEKSTART", "THISWEEKSTART"].indexOf(
				dateUpper) >= 0) {
		...
		} else if (["M", "TM", "MONTH", "THISMONTH", "MONTHSTART", "THISMONTHSTART"]
			.indexOf(dateUpper) >= 0) {
			ret.setDate(1);
		...
		} else if (["Q", "TQ", "QUARTER", "THISQUARTER", "QUARTERSTART",
				"THISQUARTERSTART"
			].indexOf(dateUpper) >= 0) {
			ret.setDate(1);
			ret.setMonth(Math.floor((ret.getMonth()) / 3) * 3);

		} else if (["LQ", "LASTQUARTER", "LASTQUARTERSTART"].indexOf(dateUpper) >=
			0) {
			ret.setDate(1);
			ret.setMonth(Math.floor((ret.getMonth()) / 3) * 3 - 3);

		} else if (["NQ", "NEXTQUARTER", "NEXTQUARTERSTART"].indexOf(dateUpper) >=
			0) {
			ret.setDate(1);
			ret.setMonth(Math.floor((ret.getMonth()) / 3) * 3 + 3);
	...
	this._getDate = function(val, format) {
		...
		while (i_format < format.length) {
			...
			if (token == "yyyy" || token == "yy" || token == "y") {
				...
				month = 0;
				var names = (token == "MMMM" ? (Date.monthNames.concat(Date.monthAbbreviations)) :
			...
			} else if (token == "E" || token == "EE" || token == "EEE" || token ==
				"EEEE") {
...
Date.prototype.getWeekNumber = function() {
	...
	return Math.floor((Math.round((this - januaryFirst) / 86400000) +
		januaryFirst.getDay() - Date.firstDayOfWeek) / 7);
};
...
Date.prototype.add = function(interval, number) {
	if (typeof(interval) == "undefined" || interval == null || typeof(number) ==
...
Date.prototype.setFirstDayOfThisWeek = function(firstDayOfWeek) {
	...
	this.setDate(this.getDate() - this.getDay() + firstDayOfWeek - (this.getDay() ==
		0 && firstDayOfWeek != 0 ? 7 : 0));
	return this;
};
...
function getMillisInDaysHoursMinutes(millis) {
	...
	var min = Math.floor((millis - days * millisInWorkingDay - hour * 3600000) /
		60000);
	return (sgn >= 0 ? "" : "-") + (days > 0 ? days + "  " : "") + pad(hour, 1,
		"0") + ":" + pad(min, 2, "0");
}
...
function millisFromString(string, considerWorkingdays) {
	...
	var regex = new RegExp(
		"([0-9\\.,]+[Yy])|([0-9\\.,]+[M])|([0-9\\.,]+[Ww])|([0-9\\.,]+[Dd])|([0-9\\.,]+[Hh])|([0-9\\.,]+[m])|([0-9\\.,]+[Ss])|(\\d+:\\d+)|(:\\d+)|(\\d*[\\.,]\\d+)|(\\d+)",
		"g");
	...
	while (matcher != null) {
		for (var i = 1; i < matcher.length; i++) {
			...
			if (match) {
				...
				if (i == 1) { // years
					totMillis = totMillis + number * (considerWorkingdays ? millisInWorkingDay *
						workingDaysPerWeek * 52 : 3600000 * 24 * 365);
				} else if (i == 2) { // months
					totMillis = totMillis + number * (considerWorkingdays ? millisInWorkingDay *
						workingDaysPerWeek * 4 : 3600000 * 24 * 30);
				} else if (i == 3) { // weeks
					totMillis = totMillis + number * (considerWorkingdays ? millisInWorkingDay *
						workingDaysPerWeek : 3600000 * 24 * 7);
				} else if (i == 4) { // days
					totMillis = totMillis + number * (considerWorkingdays ? millisInWorkingDay :
						3600000 * 24);
				} else if (i == 5) { // hours
...
function daysFromString(string, considerWorkingdays) {
	...
	var regex = new RegExp(
		"([0-9\\.,]+[Yy])|([0-9\\.,]+[Mm])|([0-9\\.,]+[Ww])|([0-9\\.,]+[Dd])|(\\d*[\\.,]\\d+)|(\\d+)",
		"g");
	...
	while (matcher != null) {
		for (var i = 1; i < matcher.length; i++) {
			...
			if (match) {
				...
				if (i == 1) { // years
					totDays = totDays + number * (considerWorkingdays ? workingDaysPerWeek *
						52 : 365);
				} else if (i == 2) { // months
					totDays = totDays + number * (considerWorkingdays ? workingDaysPerWeek * 4 :
						30);
				} else if (i == 3) { // weeks
